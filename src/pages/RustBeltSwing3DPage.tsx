...
// Helper to construct asset URLs that work with Vite's base path
...
const RustBeltSwing3DPage: React.FC = () => {
  ...
  async function loadFallbackYear(year: number): Promise<Map<string, CountyBaseline>> {
    ...
    try {
      const url = getAssetPath(`data/results/counties_${year}.json`)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const s = await fetch(getAssetPath('gz_2010_us_040_00_500k.json')).then(r=>r.json())
        if (!alive) return
        ...
        const c = await fetch(getAssetPath('gz_2010_us_050_00_500k.json')).then(r=>r.json())
        if (!alive) return
